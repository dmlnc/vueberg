    ...
    <div class="gutentap-modal-layout">
        ...
        <div class="gutentap-modal" :class="`gutentap-modal-${size}`">
            ...
            <div class="gutentap-modal-body">
              <div class="gutentap-form">
                <div 
                    class="gutentap-form-item"
                    v-for="(item, index) in form"
                    :key="index"
                >
                    <label :for="`gutentap-modal-input-${item.name}-${index}`">{{ item.label }}</label>
                    <input :id="`gutentap-modal-input-${item.name}-${index}`" :placeholder="item.placeholder" :autofocus="index == 0 ? true : false" class="gutentap-form-control" type="text" v-model="item.value">
                    <small v-if="item.text" v-html="item.text"></small>
                </div>
                <div class="gutentap-form-buttons">
                    <menu-button
